...
case class SeqCodec[ELEM, TO](
    builderMethod: Method,
    companionInstance: Object,
    elementDecoder: Decoder[ELEM],
    elementEncoder: Encoder[ELEM])
    extends Codec[TO]:

  lazy val decoder: Decoder[TO] = new Decoder[TO] {
    private var inArray = false
    private var arrayDone = false

    private val builder = builderMethod
      .invoke(companionInstance)
      .asInstanceOf[scala.collection.mutable.Builder[ELEM, TO]]
    ...
    def emit(token: ParseToken, parser: Parser): Either[EmitResult, TO] =
      if arrayDone then error("Unexpected content after array end", parser)
        ...
        (elementDecoder.emit(token, parser): @switch) match {
          ...
          case Left(EmitResult.REJECTED) =>
            ...
            else error("Unexpected token " + token, parser)
        }
      else
        token match {
  ...
  lazy val encoder: Encoder[TO] = new Encoder[TO] {

    def encode(payload: TO, writer: Writer[_]): Unit =
      if payload == null then writer.writeNull()
      else
        writer.writeArray[ELEM](
          payload.asInstanceOf[Seq[ELEM]].toList,
          elementEncoder
        )
  }
